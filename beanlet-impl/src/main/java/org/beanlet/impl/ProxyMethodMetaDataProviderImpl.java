...
import org.beanlet.impl.metadata.ProxyMethodMetaDataImpl;
 ...
 */
public final class ProxyMethodMetaDataProviderImpl extends AbstractProvider 
    ...
    public List<MetaData> getMetaData(ComponentConfiguration configuration) {
        ...
        if (configuration instanceof BeanletConfiguration) {
            AnnotationDomain domain = ((BeanletConfiguration) configuration).
                    getAnnotationDomain();
            Method method = ProxyMethods.getInstance(configuration).
                    getProxyMethod();
            if (method != null) {
                ProxyMethod annotation = domain.getDeclaration(
                        ProxyMethod.class).getAnnotation(
                ...
                List<Annotation> annotations = 
                        ...
                        getAnnotationDomain().getAnnotations(
                        MethodElement.instance(method));
                metaData.add(new ProxyMethodMetaDataImpl(
                        getAnnotatedElement(annotations), description));
            }
        }
