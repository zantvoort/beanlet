 ...
 */
public final class WireByNameDependencyInjectionFactoryImpl extends
        WiringDependencyInjectionFactory {
    
    private final BeanletConfiguration<?> configuration;
    
    public WireByNameDependencyInjectionFactoryImpl(
    ...
    public boolean isSupported(ElementAnnotation<? extends Element, Inject> ea) {
        if (isWiringModeSupported(ea)) {
            ...
            if (configuration.getComponentName().equals(name)) {
                ...
                return BeanletApplicationContext.instance().exists(name);
            }
        } else {
            return false;
        }
    ...
    public Set<String> getDependencies(
        ...
        if (domain.getDeclaration(IgnoreDependency.class).
            ...
            dependencies = Collections.singleton(getName(ea));
        }
        return dependencies;
    }
