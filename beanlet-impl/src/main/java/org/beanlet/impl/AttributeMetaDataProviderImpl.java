...
import org.beanlet.impl.metadata.AttributeReadMetaDataImpl;
import org.beanlet.impl.metadata.AttributeWriteMetaDataImpl;
...
import org.beanlet.annotation.Elements;
 ...
 */
public final class AttributeMetaDataProviderImpl extends AbstractProvider 
    ...
    public List<MetaData> getMetaData(ComponentConfiguration configuration) {
        ...
        if (configuration instanceof BeanletConfiguration) {
            AnnotationDomain domain = ((BeanletConfiguration) configuration).
                    getAnnotationDomain();
            Attributes attributes = Attributes.getInstance(configuration);
            for (Getter getter : attributes.getGetters()) {
                Element element = Elements.instance(getter.getMember());
                final boolean is;
                final String operationName;
                if (getter.getMember() instanceof Method) {
                    Method method = (Method) getter.getMember();
                    is = Attributes.isIs(method);
                    operationName = method.getName();
                } else {
                    is = false;
                    operationName = null;
                }
                metaData.add(new AttributeReadMetaDataImpl(
                        getter.getName(),
            ...
            for (Setter setter : attributes.getSetters()) {
                Element element = Elements.instance(setter.getMember());
                final String operationName;
                if (setter.getMember() instanceof Method) {
                    Method method = (Method) setter.getMember();
                    operationName = method.getName();
                } else {
                    operationName = null;
                }
                metaData.add(new AttributeWriteMetaDataImpl(
                        setter.getName(),
                        getAnnotatedElement(domain.getAnnotations(element)),
                        setter.getType(), 
                        operationName,
