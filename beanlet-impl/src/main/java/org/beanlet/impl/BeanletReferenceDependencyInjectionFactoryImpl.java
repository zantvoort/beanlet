 ...
 */
public final class BeanletReferenceDependencyInjectionFactoryImpl extends
        ParameterizedTypeAwareDependencyInjectionFactory {
    
    private final BeanletConfiguration<?> configuration;
    
    public BeanletReferenceDependencyInjectionFactoryImpl(
    ...
    public boolean isSupported(ElementAnnotation<? extends Element, Inject> ea) {
        ...
        if (BeanletReference.class.isAssignableFrom(type)) {
            supported = true;
        } else {
            supported = false;
        }
    ...
    public Injectant<?> getInjectant(
        ...
        return new InjectantImpl<Object>(factory.create(), false);
    }
}
