 ...
 */
public final class NamingContextBeanletConfigurationValidatorImpl extends
        AbstractBeanletConfigurationValidator<NamingContext> {
    
    public Class<NamingContext> annotationType() {
    ...
    private void checkInjection(BeanletConfiguration<?> configuration, 
        ...
        if (result) {
            throw new BeanletValidationException(
                    ...
                    "marked for JNDI wiring: '" + element + "'.");
        }
    }
        ...
        if (!element.getElementType().equals(ElementType.PACKAGE) &&
            ...
            if (inject == null) {
                if (element instanceof ParameterElement || 
                    ...
                    throw new BeanletValidationException(
                            configuration.getComponentName(),
                            "Member marked with NamingContext annotation MUST also " +
                    ...
                    if (!hasInject) {
                        throw new BeanletValidationException(
                                configuration.getComponentName(),
                                "Member marked with NamingContext annotation " +
