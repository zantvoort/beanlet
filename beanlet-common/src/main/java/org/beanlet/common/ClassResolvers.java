 ...
 */
public final class ClassResolvers implements ClassResolver {
    ...
    public ClassResolvers(List<ClassResolver> resolvers) {
        if (resolvers == null) {
    ...
    public List<Class> getClasses(AnnotationDomain domain) {
        LinkedHashSet<Class> set = new LinkedHashSet<Class>();
        for (ClassResolver resolver : resolvers) {
            set.addAll(resolver.getClasses(domain));
        }
